...
public class AnagramGenerator {
	...
	public List<Anagram> generateAnagram(String input){
		...
		for(String word : lexicon) {											// For each word in our lexicon list...
				...
				if(wordAnagrams.size() != 0)	anagrams.add(generateAnagram(baseCount, word, wordAnagrams));	// Recursively add valid anagrams
			}
		...
		if(input.isSubset(wordAlphaCount)) {						// If the word is a subset of input...
			...
			if(!input.isEmpty()) {
				for(String nextWord : lexicon) {						// For all other words in the lexicon...
					...
					if(newAnagram != null) wordAnagrams = newAnagram;
					//if(newAnagram != null) wordAnagrams = wordAnagrams.addAnagram(newAnagram);		// Add Anagram thats generated by recursive call to our Anagram
				}
			}
		}
