...
public class ULHashMap<K,V> implements Cloneable, Iterable<ULHashMap.Mapping<K, V>>{
	...
	public void insert(K key, V value) {
		...
		//Else, iterate through bucket to see if K is already there
			...
			for(Mapping<K,V> mapping : buckets[bucketIndex]) {
				if(mapping.key.equals(key))	throw new DuplicateKeyException();
			}
			...
			if(size == tableSize) {
				...
				while(!isPrime(newTableSize)) ++newTableSize;
	...
	public boolean isPrime(int possibleSize) {
		...
		// While we have more numbers to check AND we haven't already found that possibleSize is not prime...
		while(possibleFactor < possibleSize && retVal == true) {
			// Check if possibleFactor is actually a factor and if not increment by one
			if(possibleSize % possibleFactor == 0)	retVal = false;
			possibleFactor++;
		}
	...
	public Iterator<ULHashMap.Mapping<K,V>> iterator() {
		return new Iterator<ULHashMap.Mapping<K,V>>(){
			...
			public Mapping<K, V> next() {
				...
				Mapping<K,V> iter = buckets[0].getFirst(); // Start at very first element of first bucket
	...
	public V lookup(K key) {
		...
		if(buckets[bucketIndex].size() == 1) retVal = buckets[bucketIndex].getFirst().getValue();
		else {
			for(Mapping<K,V> mapping : buckets[bucketIndex]) {
				if(mapping.getKey().equals(key))	retVal = mapping.getValue();
			}
		}
